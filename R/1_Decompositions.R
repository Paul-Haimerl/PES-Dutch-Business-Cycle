runHP <- function(y, boosted = TRUE) {
  HPResult <- BoostedHP(y, lambda = 1600, stopping = "BIC", iter = boosted)
  trend <- HPResult$trend
  cycle <- HPResult$cycle

  outputMat <- cbind(trend = trend, cycle = cycle)
  return(outputMat)
}
